...
-> Should work when more than 1 case.
...
purrr::walk(assay_sel_mean, \(cur_assay) {
  h <- dittoHeatmap(sce_mean, assay = cur_assay,
                    ...
                    main = paste("Heatmap", "NotScaled", cur_assay,
                                 sep = " - "),
                    annotation_colors = list(case_id = palettes$cases[1],
                                             donor_type = palettes$stages[1]),
                    genes = channels)
...
```{r display-images}
library(cytoviewer)
spe_sub[[clust_name]] <- as.factor(spe_sub[[clust_name]])
