...
spe_cat <- spe[channels_cat, metadata(spe)[["subset"]]]
```
...
reducedDim(spe_sub, dimred_name1) <- results[[1]]
reducedDim(spe_sub, dimred_name2) <- results[[2]]
```

## Run UMAP for Categories.
...
## Add UMAPs to SPE object
dimred_name1 <- paste("UMAP", assay_sel[1], "cat", sep = "_")
dimred_name2 <- paste("UMAP", assay_sel[2], "cat", sep = "_")

reducedDim(spe_cat, dimred_name1) <- results[[1]]
reducedDim(spe_cat, dimred_name2) <- results[[2]]
```
...
# Prepare the data
if (cur_assay %in% assayNames(spe_sub)) {
  cur_dat <- scuttle::makePerCellDF(spe_sub, features = channels_dimred,
                                    exprs_values = cur_assay) |>
                dplyr::select(c(all_of(channels_dimred), paste0(dimred_name, ".1"), 
...
cur_dat <- cur_dat |>
    tidyr::pivot_longer(cols = all_of(channels_dimred),
                        names_to = "channel",
                        values_to = cur_assay)
  
  # Plot marker expression
p <- plot_dim_red_channels(cur_dat, dimred_name, cur_assay,
                              channels_dimred, force_points = TRUE)
