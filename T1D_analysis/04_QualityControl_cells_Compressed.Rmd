...
purrr::map(.x = assay_sel, \(cur_assay) {
  cur_dat <- dplyr::bind_cols(
    case_id = colData(spe_sub)$case_id,
    t(assay(spe_sub, cur_assay))
  )
...
Q: Do you observe differences in the 90th percentile between cases and stages?
...
Aggregated both by stage and case.
...
purrr::map2(assay_sel, seq_along(assay_sel), \(cur_assay, idx_assay) {
  purrr::map2(plot_variables2, seq_along(plot_variables2), \(plot_x, idx_plots) {
    ...
    # Plot heatmap
    hmlist[[idx_plots]][[idx_assay]] <- heatmaply::heatmaply(
...
```{r plot-dimred, fig.width=15, fig.height=10}
# Prepare the data
cur_dat <- makePerCellDF(spe_cat[, colnames(spe) %in% metadata(spe)$subset]) |>
  dplyr::mutate(case_id = factor(case_id, levels = cases),
                donor_type = factor(donor_type, levels = stages)) |>
  dplyr::arrange(case_id, donor_type)
cur_dimred <- "UMAP"
